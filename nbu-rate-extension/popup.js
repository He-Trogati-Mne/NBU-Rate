...
const AUTO_REFRESH_MS = 10 * 60 * 1000; // автооновлення кожні 10 хв
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Автооновлення, поки вікно розширення відкрите
  setInterval(fetchRates, AUTO_REFRESH_MS);

  chrome.storage.onChanged.addListener(async (changes, area) => {
    if (area === 'sync') {
      if (changes.theme) await applyThemeValue(changes.theme.newValue);
      if (changes.currencies) fetchRates();
      if (changes.showPreciseRate) fetchRates();
      if (changes.language) {
    ...
    // Фонове оновлення — миттєво перемальовуємо
    if (area === 'local' && changes.cachedRates && Array.isArray(changes.cachedRates.newValue)) {
    ...
    html += `
      <div class="rate-row">
        ...
        <div style="display: flex; align-items: center; justify-content: flex-end;">
            <span class="rate-value ${rateClass}">${item.rate.toFixed(decimals)} ₴</span>
