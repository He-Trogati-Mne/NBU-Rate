const ALL_CURRENCIES = ['USD', 'EUR', 'GBP', 'PLN', 'CNY', 'CHF', 'JPY', 'CAD', 'AUD', 'TRY'];
...
let currentLang = 'uk';
let currentTheme = 'system';   // кеш, щоб не читати storage на кожен рух мишки
...
async function initTheme() {
  ...
  document.querySelectorAll('#theme-switch button').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      // 2. Потім зберігаємо (у try/catch, щоб не ламати UI при помилці)
      try {
...
/** Застосовує колір у UI ОДРАЗУ, а зберігає з невеликою затримкою. */
let saveTimer;
function previewAndSave() {
  const colors = readCustomColors();

  // 1. Миттєвий візуальний фідбек
  if (currentTheme === 'custom') {
    applyCustomColors(colors);
  }

  // 2. Дебаунс-збереження (щоб не спамити storage)
